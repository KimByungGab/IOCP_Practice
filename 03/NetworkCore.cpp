...
ClientInfo* NetworkCore::GetEmptyClientInfo()
{
	...
	{
		if (client.mClientSocket == INVALID_SOCKET)
			return &client;
	}
...
bool NetworkCore::BindIoCompletionPort(ClientInfo* pClientInfo)
{
	...
	HANDLE hIOCP = CreateIoCompletionPort((HANDLE)pClientInfo->mClientSocket, mIOCPHandle, (ULONG_PTR)pClientInfo, 0);
...
void NetworkCore::WorkerThread()
{
	...
	while (mIsWorkerRun)
	{
		...
		{
			cout << "클라 접속 종료! " << (int)pClientInfo->mClientSocket << endl;
		...
		case IOOperation::RECV:
			...
			// 끝에 NULL(\0) 넣어줘서 문자열이 끝났다는 것을 알려줌
			pClientInfo->mRecvBuf[dwIoSize] = NULL;
			cout << "[수신] bytes: " << dwIoSize << ", msg: " << pClientInfo->mRecvBuf << endl;
		...
		case IOOperation::SEND:
			cout << "[송신] bytes: " << dwIoSize << ", msg: " << pClientInfo->mSendBuf << endl;
			break;

			// 예외 처리
		default:
			cout << "socket(" << (int)pClientInfo->mClientSocket << ")에서 예외상황" << endl;
			break;
		}
	}
}
...
void NetworkCore::AcceptThread()
{
	...
	while (mIsAcceptRun)
	{
		...
		// 클라 접속 요청이 들어올때까지 대기
		pClientInfo->mClientSocket = accept(mListenSocket, (SOCKADDR*)&stClientAddr, &nAddrLen);
		if (pClientInfo->mClientSocket == INVALID_SOCKET)
			continue;
		...
		cout << "클라 접속: IP(" << clientIP << ") SOCKET(" << (int)pClientInfo->mClientSocket << ")" << endl;
...
{
	int clientIndex = pClientInfo->mIndex;
	...
	// 소켓의 send recv 전부 중단
	shutdown(pClientInfo->mClientSocket, SD_BOTH);

	// 소켓 옵션을 지정해둔 linger로 설정
	setsockopt(pClientInfo->mClientSocket, SOL_SOCKET, SO_LINGER, (char*)&stLinger, sizeof(stLinger));

	// 소켓 종료
	closesocket(pClientInfo->mClientSocket);

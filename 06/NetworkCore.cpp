...
bool NetworkCore::CreateSendThread()
{
	mIsSendRun = true;
	mSendThread = thread([this]()
		{
			SendThread();
		});
	return false;
...
void NetworkCore::WorkerThread()
{
	...
	while (mIsWorkerRun)
	{
		...
		case IOOperation::RECV:
			...
			PushPacketData(pClientInfo->GetIndex(), dwIoSize, pClientInfo->GetRecvBuffer());
...
// Send 쓰레드
void NetworkCore::SendThread()
{
	while (mIsSendRun)
	{
		// 가장 최근의 패킷 전달받기
		Packet packet = GetPacketData();

		// 패킷이 존재한다면
		if (packet.mDataSize != 0)
		{
			SendMsg(GetClientInfo(packet.mSessionIndex), packet.m_pPacketData.c_str(), packet.mDataSize);
		}
		else
		{
...
// 패킷 데이터 받기
Packet NetworkCore::GetPacketData()
{
	// 패킷
	Packet packet;
	...
	Packet pPacket = mPacketQueue.front();
	mPacketQueue.pop();
...
// 패킷을 큐에 삽입
void NetworkCore::PushPacketData(const unsigned int clientIndex, const unsigned int size, char* pData)
{
	Packet packet;
